...
def main():
    ...
    if not os.path.exists(file_path):
        ...
        file_path = '/home/student/Downloads/housing.csv'
        if not os.path.exists(file_path):
            print(f"Error cannot find: housing.csv")
            return 
            
    df=pd.read_csv(file_path, sep='\t')
    df.columns = df.columns.str.strip()  # remove any leading/trailing spaces
    
    # Map housing columns to pipeline columns
    if 'MedInc' in df.columns:
        ...
        df = df.rename(columns={
            'total_bedrooms': 'H',
            'total_rooms': 'R',
            'median_house_value': 'W',
            'housing_median_age': 'HR',
            'population': 'SO',
            'households': 'SB',
        })
        df['ID'] = range(1, len(df) + 1)
